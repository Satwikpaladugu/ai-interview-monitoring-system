...
A comprehensive FastAPI-based system for real-time interview monitoring using facial recognition, embeddings-based identity verification, and environmental quality detection.
...
- 65% threshold for confident matches
...
```
image detection/
├── app.py                    # FastAPI backend runner
├── app_fastapi.py            # FastAPI app, endpoints, and verification logic
...
**Cause:** FastAPI server not running or different port.
...
2. For HTTPS, run FastAPI behind an HTTPS-capable proxy/server
...
**Cause:** CORS middleware not allowing requests.

**Solution:**
1. Verify FastAPI CORS middleware is configured in `app_fastapi.py`
2. Check URL in index.html:
   ```javascript
   const API = "http://127.0.0.1:5000";
   ```
3. Ensure FastAPI is actually running on that address/port
